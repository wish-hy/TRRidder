...
class TRScrollMenuView: UIView {
    var currentSel : Int = 0 {
        didSet{
           configChangeState(state: currentSel)
        }
    }
    var model : TROrderCountModel = TROrderCountModel() {
        didSet {
            menuLab1.text = "新任务(\(model.waitingCount))"
            menuLab2.text = "待取货(\(model.takeCount))"
            menuLab3.text = "配送中(\(model.deliveringCount))"
            self.layoutIfNeeded()
        }
    }
    ...
    private func setupView(){
        ...
        menuLab1.font = UIFont.trBoldFont(fontSize: 20)
        self.addSubview(menuLab1)
        
        menuLab2 = UILabel()
        menuLab2.text = "待取货(0)"
        menuLab2.textColor = UIColor.hexColor(hexValue: 0xE2FFF0)
        menuLab2.font = UIFont.trFont(fontSize: 16)
        self.addSubview(menuLab2)
        
        menuLab3 = UILabel()
        menuLab3.text = "配送中(0)"
        menuLab3.textColor = UIColor.hexColor(hexValue: 0xE2FFF0)
        menuLab3.font = UIFont.trFont(fontSize: 16)
