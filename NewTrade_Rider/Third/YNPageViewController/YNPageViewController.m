...
- (void)yn_pageScrollViewDidScrollView:(UIScrollView *)scrollView {
    //-_config.pageHeaderViewOriginHeight
    if (_config.limitScroll) {
        if (scrollView.contentOffset.y <= -(self.headerViewOriginHeight + _config.menuHeight)){
            scrollView.contentOffset = CGPointMake(0, -(self.headerViewOriginHeight + _config.menuHeight));
        }
    }
    if ([self isSuspensionBottomStyle] || [self isSuspensionTopStyle]) {
...
- (void)setSelectedPageIndex:(NSInteger)pageIndex {
    ...
    if (frame.origin.x == self.pageScrollView.contentOffset.x) {
        ...
        [self.pageScrollView scrollRectToVisible:frame animated:false];
    }
