...
- (void)setupOtherViews {
    ...
    self.scrollView.frame = CGRectMake(0, 0, self.configration.showAddButton ? self.yn_width - 30 : self.yn_width, self.yn_height);
    
    [self addSubview:self.scrollView];
    
    if (self.configration.showAddButton) {
        self.addButton.frame = CGRectMake(self.yn_width - 64 - 10, 8, 64, 32);
    ...
    CGFloat imageLineY = self.scrollView.yn_height - self.configration.lineImageHeight;

    CGFloat lineW = [[self.itemsArrayM firstObject] yn_width];
    CGFloat lineH = self.configration.lineHeight;
    CGFloat imageLineH = self.configration.lineImageHeight;
    ...
    if (self.configration.showScrollImageLine) {
        self.lineImageView.frame = CGRectMake(lineX - self.configration.lineLeftAndRightAddWidth + self.configration.lineLeftAndRightMargin, imageLineY - 8, 18, 10);
//        self.lineImageView.layer.cornerRadius = self.configration.lineCorner;
...
- (void)setDefaultTheme {
    ...
    if (self.configration.showScrollImageLine) {
        self.lineImageView.yn_x = currentButton.yn_x - self.configration.lineLeftAndRightAddWidth + self.configration.lineLeftAndRightMargin;
        self.lineImageView.yn_width = currentButton.yn_width + self.configration.lineLeftAndRightAddWidth *2 - self.configration.lineLeftAndRightMargin * 2;
        
        
        if (!self.configration.scrollMenu &&
            !self.configration.aligmentModeCenter &&
            self.configration.lineWidthEqualFontWidth) { /// 处理Line宽度等于字体宽度
            self.lineImageView.yn_x = currentButton.yn_x + ([currentButton yn_width]  - ([self.itemsWidthArraM[currentButton.tag] floatValue])) / 2 - self.configration.lineLeftAndRightAddWidth - self.configration.lineLeftAndRightAddWidth;
            self.lineImageView.yn_width = [self.itemsWidthArraM[currentButton.tag] floatValue] + self.configration.lineLeftAndRightAddWidth *2;
        }
    }
...
- (void)adjustItemAnimate:(BOOL)animated {
    ...
    [UIView animateWithDuration:animated ? 0.3 : 0 animations:^{
        ...
        if (self.configration.showScrollImageLine) {
            self.lineImageView.yn_x = currentButton.yn_x - self.configration.lineLeftAndRightAddWidth + self.configration.lineLeftAndRightMargin;
            self.lineImageView.yn_width = currentButton.yn_width + self.configration.lineLeftAndRightAddWidth * 2 - 2 * self.configration.lineLeftAndRightMargin;
            
            if (!self.configration.scrollMenu &&
                !self.configration.aligmentModeCenter &&
                self.configration.lineWidthEqualFontWidth) {//处理Line宽度等于字体宽度
                
                self.lineImageView.yn_x = currentButton.yn_x + ([currentButton yn_width]  - ([self.itemsWidthArraM[currentButton.tag] floatValue])) / 2 - self.configration.lineLeftAndRightAddWidth;;
                self.lineImageView.yn_width = [self.itemsWidthArraM[currentButton.tag] floatValue] + self.configration.lineLeftAndRightAddWidth *2;
            }
...
- (void)adjustItemWithProgress:(CGFloat)progress
    ...
    if (self.configration.showScrollImageLine) {
        ...
        if (!self.configration.scrollMenu &&
            !self.configration.aligmentModeCenter &&
            self.configration.lineWidthEqualFontWidth) { /// 处理Line宽度等于字体宽度
            self.lineImageView.yn_x = lastButton.yn_x + ([lastButton yn_width]  - ([self.itemsWidthArraM[lastButton.tag] floatValue])) / 2 - self.configration.lineLeftAndRightAddWidth + xD *progress;
            
            self.lineImageView.yn_width = [self.itemsWidthArraM[lastButton.tag] floatValue] + self.configration.lineLeftAndRightAddWidth *2 + wD *progress;
            
        } else {
            self.lineImageView.yn_x = lastButton.yn_x + xD *progress - self.configration.lineLeftAndRightAddWidth + self.configration.lineLeftAndRightMargin;
            self.lineImageView.yn_width = lastButton.yn_width + wD *progress + self.configration.lineLeftAndRightAddWidth *2 - 2 * self.configration.lineLeftAndRightMargin;
        }
    }
    /// 遮盖
    if (self.configration.showConver) {
...
- (UIImageView *)lineImageView {
    if (!_lineImageView) {
        ...
        _lineImageView.image = [UIImage imageNamed:self.configration.lineImageName];
    }
...
//        [_addButton setImage:[UIImage imageNamed:self.configration.addButtonHightImageName] forState:UIControlStateHighlighted];
        [_addButton setBackgroundImage:[UIImage imageNamed:self.configration.addButtonHightImageName] forState:UIControlStateNormal];
