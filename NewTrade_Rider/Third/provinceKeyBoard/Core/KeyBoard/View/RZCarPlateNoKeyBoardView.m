...
// 是否是iPhone X
#define rz_kiPhoneX [UIApplication sharedApplication].statusBarFrame.size.height > 20
// 底部安全边距
#define rz_kSafeBottomMargin (rz_kiPhoneX ? 0.f: 0.f)
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    frame = CGRectMake(0, 0, rz_kScreenWidth, self.rz_cellHeight * 4 + 10 + 44);
    if (self = [super initWithFrame:frame]) {
        UICollectionViewFlowLayout *layout = [[UICollectionViewFlowLayout alloc] init];
        layout.minimumLineSpacing = 0;
        layout.minimumInteritemSpacing = 0;
        
        self.collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 30, rz_kScreenWidth, self.rz_cellHeight * 4 + 10) collectionViewLayout:layout];
...
- (void)layoutSubviews {
    ...
    self.collectionView.frame =  CGRectMake(0, 44, rz_kScreenWidth, self.rz_cellHeight * 4 + 10);
