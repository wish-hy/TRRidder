...
- (void)jumpSetting
{
    [TKPermissionPublic alertPromptTips:@"为了让您可以处理附近的订需要访问您的位置"];
}
...
/**
 查询是否获取了仅在使用应用期间位置权限
 **/
+ (CLAuthorizationStatus)checkAuth
{
    CLAuthorizationStatus isAuth = kCLAuthorizationStatusNotDetermined;
...
+ (void)authWithAlert:(BOOL)isAlert completion:(void(^)(BOOL isAuth))completion
{
    ...
    obj.isAlert = isAlert;
    CLAuthorizationStatus status = [self checkAuth];
    if (status != kCLAuthorizationStatusNotDetermined) {
        [obj handleStatus:status];
        return;
    }
...
- (void)handleStatus:(CLAuthorizationStatus)status
{
    if (status == kCLAuthorizationStatusNotDetermined) {//第一次弹出授权页面，不处理
        ...
        if (status == kCLAuthorizationStatusAuthorizedAlways || status == kCLAuthorizationStatusAuthorizedWhenInUse || status == kCLAuthorizationStatusRestricted) {
